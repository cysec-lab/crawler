...
from decision_tree_tag import learning, prediction, preparation, split
...
def main(path, sample, sample2):
    ...
    for target_server, v in sorted(target_server_quantity.items(), key=lambda x: x[1], reverse=True):
        ...
        # 学習
        classifier = learning.learning_main(data_train, label_train, best_estimator)
        tree_dict[target_server[0:target_server.find('.')]] = classifier
