...
class CheckSearchedUrlThread(Thread):
    ...
    def run(self):
        ...
        host_name = urlparse(check_url).netloc
        ...
        if not host_name:      # URL内に「://」がなかった場合
            ...
            for i in range(1):
                for white in self.white_list:  # ホワイトリストはホスト名は組織ではないが、URLが途中まで一致したら接続可とする
                    ...
                    if host_name.endswith(white_host):
                        if white_url in check_url:
                            crawling_flag = False         # 一度falseを入れた後、もう一度trueを入れてループから抜ける
                            break
                if crawling_flag is False:
                    crawling_flag = True
                    break
                # 立命館ではないと分かっているため、ファイルに出力しない
                if [not_domain for not_domain in self.not_domain_list if host_name.endswith(not_domain)]:
                    crawling_flag = False  # 立命館関連のサイトが多すぎて、外部だと判断されたものを
                    break                  # 出来る限り目で見て本当に外部か判断していたため。それの作業用
                if [black_string for black_string in self.black_list if black_string in check_url]:  # ブラックリストを避ける
                    crawling_flag = 'black'
                    break
                # ドメイン名が設定されているものならば
