...
# IPアドレスのチェックだけスレッドで行う(時間がかかる可能性があるため)
...
# "診断結果"にはそれぞれ、　クローリング対象URL： True、　組織内だがブラックリストでクローリングしないURL： "Black"、　組織外URL：　False、　
# IPアドレス検査をしようとして失敗したURL： "Unknown"、　リンクやリクエストなどの特別ホワイトリストによってフィルタされたURL： "Special"　が入る
